if not game:IsLoaded() then game.Loaded:Wait() end
        ...
        task.spawn(function()
            ...
            if isAirborne then
                ...
                if r then
                    ...
                    r.CFrame = CFrame.new(Vector3.new(r.Position.X, gY + 2.5, r.Position.Z))
                ...
                while not aborted() do
                    ...
                    r.AssemblyLinearVelocity = Vector3.new(
                        r.AssemblyLinearVelocity.X, -120, r.AssemblyLinearVelocity.Z)
                    task.wait(0.03)
                end
            end
            if aborted() then return end
    ...
    RunService.Heartbeat:Connect(function()
        ...
        if not AutoLREnabled and not AutoBatEnabled and hum.MoveDirection.Magnitude > 0 then
    ...
    local _, _alrSetState, _alrSetSide = FeatToggleWithSide("Auto", 7,
        ...
        function(s, side)
            AutoLREnabled = s
            if not s then AutoLRRunning = false end
        end,
        function(side) switchSide(side) end
    )
    table.insert(_sideRowRefs, { setSide = _alrSetSide })
    autoLRSetState = _alrSetState
    local _, _float2Set = FeatToggle("Float", 8, function(s)
        handleFloat2Toggle(s, nil)
    end)
    float2SetState = _float2Set
    local _, _dropSet = FeatToggle("Drop Brainrot", 9, function(s)
        handleBrainrotToggle(s, _dropSet)
    end)
    dropBrainrotSetState = _dropSet
    FeatSectionHeader("DYNAMICS", 10)
    local _, noAnimSetState = FeatToggle("No Animation", 11, function(s)
    ...
    local _, fpsSetState = FeatToggle("FPS Boost", 12, function(s)
    ...
    local _, espSetState = FeatToggle("ESP", 13, function(s)
        Config.ESPEnabled = s
    ...
    local _, ragdollSetState = FeatToggle("No Ragdoll", 14, function(s)
        Config.AntiRagdollEnabled = s
        if s then startAntiRagdoll() else stopAntiRagdoll() end
    end)
    local _, grabSetState = FeatToggle("Grab", 15, function(s)
        GrabActive = s; Config.GrabState = s
        if not s then Interacting = false; PillFill.Size = UDim2.new(0, 0, 1, 0); BarPctLbl.Text = "0" end
    end)
    local _, infJumpSetState = FeatToggle("Infinite Jump", 16, function(s)
        Config.InfJumpEnabled = s
    end)
    local _, noclipSetState = FeatToggle("Noclip Players", 17, function(s)
    ...
    SetKeybindRow("Auto",      "AutoKey",    5)
    SetSectionHeader("VALUES", 6)
    SetInputRow("Speed",        "FastSpeed",    7)
    SetInputRow("Float Height", "Float2Height", 8)
    do
        Config.BatDist        = 0
        Config.Step2Delay     = 0.05
        Config.FloatHeight    = 18
